...
extend(Chart.prototype, {
	 ...
	 * @param {Object} chartOptions Additional chart options for the SVG representation of the chart
	 */
	exportChartAsCSV: function (options, chartOptions) {
		var exportingOptions = this.options.exporting,
			chart = this,
			svg = "CSVDATA(";
		
		for (var i = 0; i < chart.series.length-1; i++){
			svg += "["+chart.series[i].name + ";";
			each(chart.series[i].yData, function (data) {
				svg += data+";"
			});
			svg = svg.substring(0,svg.length-1);
			svg +="@";
			each(chart.series[i].xData, function (data) {
				svg += data+";"
			});
			svg += "]/";
		}
		svg += "[Date;";
		each(chart.series[0].xData, function (data) {
			svg += data+";"
		});
		svg = svg.substring(0,svg.length-1);
		svg += "])";
...
//Create the csv icon
symbols.csvIcon = function (x, y, width, height) {
	return crisp([
		M, // the disk
		x,y,
		L,
		x + height, y,
		x + height, y + width,
		x + height * 0.8, y + width,
		x + height * 0.8, y + width * 0.2,
		x + height * 0.6, y + width * 0.2,
		x + height * 0.6, y + width,
		x + height * 0.4, y + width,
		x + height * 0.4, y + width * 0.6,
		x + height * 0.2, y + width * 0.6,
		x + height * 0.2, y + width,
		x, y + width,
		'Z'
	]);
};
